...
import { deleteMultiStudents, deleteStudent } from 'src/store/apps/student/actions'

function ConfirmDialog({ message, open, toggle, loading, confirmationType, handleSubmit }) {
  ...
  const handleSubmit = () => {
    if (selectedStudent.length > 1) {
      dispatch(deleteMultiStudents(selectedStudent))
    } else {
